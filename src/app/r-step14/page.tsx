...
interface Counts {
  bedroom: number;
  fullBath: number;
  threeQuarterBath: number;
  halfBath: number;
  floors: number;
}

// Furnishings list with unique IDs
const furnishings: Facility[] = [
...
const societyAmenities: Facility[] = [
...
export default function FacilitiesDetail() {
  ...
  const handleCountChange = (key: keyof Counts, delta: number) => {
    setCounts((prev) => ({
      ...prev,
  ...
  return (
    <>
      ...
      <div className="relative text-gray-900 bg-gray-50 min-h-screen overflow-hidden">
        ...
        >
          <div className="w-[100%] md:w-[83%] mx-auto px-4">
            ...
            <div className="bg-white rounded-2xl shadow-md mt-6 p-4">
              ...
              {[
                { label: "Bedroom", key: "bedroom" as keyof Counts },
               
               
                
                { label: "Floor (above Ground)", key: "floors" as keyof Counts,sub: "",},
              ].map((item, index) => (
                <div
                  key={index}
  ...
  `}
                    >
                      ...
                      />
                      <span
                        className={`text-[12px] md:text-[14px] mt-2 font-medium ${
                          isSelected ? "text-[#2450A0]" : "text-[rgba(113,117,128,1)]"
                        }`}
    ...
    ${isSelected ? "border border-[#0052CC] bg-[#EBF1FF] text-[#0052CC]" 
    : "border border-[#1218280D] bg-[#f3f3f3] text-[#121212]"}`}
                      >
                        {/* icon ab hamesha same rahega */}
                        <img
                          src={item.icon}
                          alt={item.label}
                          className="w-[40px] h-[40px]"
